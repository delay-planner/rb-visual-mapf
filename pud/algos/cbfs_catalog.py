from pud.algos.cbfs_eval import catalog_precompiled_paths
import argparse
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cbfs_dir',
        type=str,
        help='the directory that contains the CBFS precompilation output')

    parser.add_argument('--output',
        type=str,
        help='the FULL path to save the output CBFS catalog file')
    args = parser.parse_args()

    cbfs_dir = Path(args.cbfs_dir)

    assert cbfs_dir.exists() and cbfs_dir.is_dir(), "cbfs_dir needs to be an existing directory"

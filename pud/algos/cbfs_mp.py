...
from pud.algos.cbfs_eval import CBFS
...
def run_CBFS(root, g_dict:dict, cost_map:np.ndarray, cost_limit:float, output_dir:Path):
    assert len(root) == 2
    out, out_cost = CBFS(g=g_dict, root=root, cost_limit=cost_limit, cost_map=cost_map)
    output_file = output_dir.joinpath("root={},{}_cost_limit={}.pkl".format(root[0], root[1], cost_limit))
    save_data = {
        "trajs": out, # list
        "costs": out_cost, # list
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_ratio',
        type=float,
        default=0.8,
        help='ratio to abuse the machine')
    ...
    parser.add_argument('--pbar', action='store_true', help='show progress bar in single-process mode')

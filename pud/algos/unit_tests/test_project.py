...
class TestClass(unittest.TestCase):
    ...
    def compute_groundtruth(self, new_zs):
        ...
        for j in range(target.shape[0]):
            ...
            target[lj] += self.probs[j]*(uj + (uj==bj).to(float)- bj).to(self.probs[j].dtype)
    ...
    def compute_in_batch(self, new_zs):
        ...
        # when lower and upper adjacent class are the same, 
        # bump up the upper bound artificially by 1 to avoid 
        # dumping the probabilities
        new_probs.scatter_add_(dim=-1, index=ls, 
                src=self.probs * (us + (us==ls).to(float) - bs).to(self.probs.dtype))
        new_probs.scatter_add_(dim=-1, index=us, src=self.probs * (bs - ls).to(self.probs.dtype))
        # test case
        return new_probs
    ...
    def project_batch(self, new_zs):
        ...
        # bump up the upper bound artificially by 1 to avoid 
        # dumping the probabilities
        out_of_range_fix = (us==ls).to(float)
        new_probs.scatter_add_(dim=-1, index=ls, 
            src=self.probs * (us + out_of_range_fix - bs).to(self.probs.dtype))
        new_probs.scatter_add_(dim=-1, index=us, 
            src=self.probs * (bs - ls).to(self.probs.dtype))
        return new_probs
    ...
    def test_pytorch_easy_mistake(self):
        ...
        The correct output is [1,0,0,0], as all elements are added to the first element, but it is easy to implement incorrectly as follows
        """
        inds = torch.LongTensor([0,0,0,0])
        probs = torch.FloatTensor([1., 0., 0., 0.])
        ...
        new_probs_wrong2.scatter_(
            dim=0, index=inds, src=new_probs_wrong2[inds] + probs
        )
        ...
        new_probs_correct.scatter_add_(dim=0, index=inds, src=probs + new_probs_correct[inds])

        self.assertTrue(torch.allclose(new_probs_correct, torch.Tensor([1., 0., 0., 0.])))

...
def extract_walls(args):
    ...
    if suffix == "npz":
        ...
        problems = pickle.load(open(args.problem_set_file, "rb"))
        if args.num_agents == 1:
            problems = [problems[0]]
        elif args.num_agents == 4:
            problems = [problems[0], problems[5], problems[6], problems[7]]
        else:
            raise ValueError(
                f"Unsupported number of agents: {args.num_agents} for visualization using illustration problems."
            )
...
def generate_wps(args, debug=False):
    ...
    if suffix == "npz":
        ...
        rb_vec = ConstrainedCollector.sample_initial_unconstrained_states(
            eval_env,
            config.replay_buffer.max_size,
            habitat=True,
        )

        agent.load_state_dict(torch.load(args.unconstrained_ckpt_file, map_location=config.device))
        agent.to(torch.device(config.device))
        agent.eval()
        pdist = agent.get_pairwise_dist(rb_vec[1], aggregate=None)  # type: ignore

        agent.load_state_dict(torch.load(args.constrained_ckpt_file, map_location=config.device))
    ...
    cbs_config = {
        ...
        "risk_bound": 4.975,  # Low is 0.18 and High is 9.77

...
def wall_plotting_fn(eval_env, ax, constrained=False):
    if not constrained:
        ...
        ax = plot_safe_walls(eval_env.walls, eval_env.get_cost_map(), eval_env.cost_limit, ax)
    return ax
...
def visualize_trajectory(agent, eval_env, difficulty=0.5, outpath=""):
    ...
    for trajectory in range(2):
        ...
        start, goal, observations, _, _, records = collector_cls.get_trajectory(agent, eval_env)
...
def visualize_policy_graph(search_policy, eval_env, outpath=""):
    ...
    plot_safe_walls(eval_env.get_map(), eval_env.get_cost_map(), cost_limit=eval_env.cost_limit, ax=ax)
        ...
        for count, j in enumerate(np.argsort(pdist_combined[i])):
            if count < edges_to_display:
                if pdist_combined[i, j] < search_policy.max_search_steps:
                    if pcost_combined is not None and pcost_combined[i, j] >= search_policy.max_cost_limit:
                        continue
...
def visualize_path(paths, filename, plot_handles, save_fig=False, waypoints=None):
    ...
    for agent in range(num_agents):
        (line,) = ax.plot([], [], "o-", ms=10, lw=2, c=AGENT_COLORS[agent][1], alpha=0.7, label=f"Agent {agent}")
        (start,) = ax.plot([], [], "x", ms=10, lw=2, c=AGENT_COLORS[agent][0], alpha=0.7, label=f"Start {agent}")
        (end,) = ax.plot([], [], "*", ms=10, lw=2, c=AGENT_COLORS[agent][0], alpha=0.7, label=f"Goal {agent}")
        if waypoints is not None:
            (waypoint_line,) = ax.plot(
                [], [], "s-", ms=10, lw=2, c=AGENT_COLORS[agent][1], alpha=0.7, label=f"Waypoint {agent}"
            )
    ...
    plt.legend(loc="lower center", bbox_to_anchor=(0.5, -0.2), ncol=num_agents, fontsize=16)
...
def visualize_search_path_single_agent(search_policy, eval_env, difficulty=0.5, outpath=""):
    ...
    if search_policy.open_loop:
        ...
        collector_cls = ConstrainedCollector if constrained else Collector
        start, goal, _, waypoints, _, _ = collector_cls.get_trajectory(search_policy, eval_env)
    ...
    ax = plot_agent_paths(0, start, goal, np.array([start, *waypoint_vec, goal]), "Search", ax, obs_marker="s")
...
def visualize_search_path_multi_agent(search_policy, eval_env, num_agents, difficulty=0.5, outpath=""):
    ...
    if search_policy.open_loop:
        ...
        collector_cls = ConstrainedCollector if constrained else Collector
        starts, goals, _, waypoints, _, _ = collector_cls.get_trajectories(search_policy, eval_env, num_agents)
...
def visualize_compare_search_single_agent(agent, search_policy, eval_env, difficulty=0.5, seed=0, outpath=""):
    ...
    for col_index in range(2):
        ...
        if not use_search:
            ax.legend(loc="lower center", bbox_to_anchor=(-0.15, -0.15), ncol=4, fontsize=16)
...
def visualize_compare_search_multi_agent(agent, search_policy, eval_env, n_agents, difficulty=0.5, seed=0, outpath=""):
    ...
    for col_index in range(2):
        ...
        if col_index == 0:
            ...
            _, _, observations, waypoints, _, records = collector_cls.get_trajectories(
                policy, eval_env, n_agents, starts, goals, start_costs=start_costs, threshold=threshold
            )
        ...
        for a_id in range(n_agents):
            ...
            waypoint_vec = np.array([a_start, *waypoints[a_id], a_goal]) if use_search else None
            ...
            ax = plot_agent_paths(a_id, a_start, a_goal, obs_vec, title, ax, waypoint_vec)

        if not use_search:
            ax.legend(loc="lower center", bbox_to_anchor=(-0.15, -0.35), ncol=n_agents, fontsize=16)
    ...
    if len(outpath) > 0:
        ...
        ax = wall_plotting_fn(eval_env, ax, constrained)
        visualize_path(search_observations, save_path, (fig, ax), save_fig=True, waypoints=search_waypoints)

...
def init_logging(cfg, model_name: str):
    logger = get_logger(
        ...
        logger_name=(
            os.getcwd() if cfg.logger.logdir == "" else os.getcwd() + cfg.logger.logdir
        ),
        experiment_name=generate_exp_name(cfg.env.walls, model_name),
        wandb_kwargs={
            "group": cfg.logger.group_name or model_name,
            "project": cfg.logger.project_name or f"torchrl_example_{cfg.env.walls}",
        },
    )
...
def log_training(
    ...
    if ("next", "agents", "reward") not in sampling_td.keys(True, True):  # type: ignore
        sampling_td.set(
            ("next", "agents", "reward"),
    ...
    if ("next", "agents", "episode_reward") not in sampling_td.keys(True, True):  # type: ignore
        sampling_td.set(
    ...
    if "info" in sampling_td.get("agents").keys():  # type: ignore
        to_log.update(
            {
                f"train/info/{key}": value.mean().item()
                for key, value in sampling_td.get(("agents", "info")).items()  # type: ignore
            }
        )
...
def log_evaluation(
    ...
    if isinstance(logger, WandbLogger):
        ...
        logger.experiment.log(
            {
                "eval/video": wandb.Video(vid, fps=1 / env_test.world.dt, format="mp4"),  # type: ignore
            },
            commit=False,
        )
    else:

...
from pud.mapf.mocbs import MultiObjectiveCBSNode
...
from pud.mapf.single_agent_planner import (
    BiObjectiveAStar,
    BiObjectiveSpaceTimeAStar,
)


class BiObjectiveCBSSolver(object):
    def __init__(
        self,
        graph: Graph,
        starts: List[int],
        ...
        for agent in range(self.num_agents):
            ...
            self.single_agent_spacetime_planners[agent] = BiObjectiveSpaceTimeAStar(
                graph, agent, starts[agent], goals[agent], config
            )
